...
class SaleOrderLine(models.Model):
    ...
    def _prepare_invoice_line_details(self, line, desc_rule):
        ...
        if desc_rule[1] == '1':
            details.append(f"{round(line.unit_amount, 3)} {line.product_uom_id.name}")
        if desc_rule[2] == '1':
    ...
    def _prepare_invoice_line(self, **optional_values):
        ...
        for line in timesheets:
            ...
            notes_by_task[line.task_id].append(' - '.join([str(x) or '' for x in details]))
        ...
        if notes_by_task and \
            ...
            for task, timesheet_lines in notes_by_task.items():
                ...
                name_lines.extend(timesheet_lines)
